...
points_after <- generate_random_points(150, x_center=3, angle_end=1.5*pi)
circle_points_before <- generate_circles()
circle_points_after <- generate_circles(x_center=3)

# Plot the circle and the points
clp_vss <- ggplot(data = circle_points_before, 
               aes(x = x, y = y)) +
  ...
  annotate("text", x=1.5, y=0.1, label="italic(t)", parse=TRUE,
           vjust=0, hjust=1, size=6) +
  annotate("segment", x = 0, y = -1, xend = 0.5, yend = -1.15, 
         linewidth=1, linejoin = "mitre", color="#004495") +
  annotate("text", x=0.5, y=-1.2, label="V[ss]", parse=TRUE,
           vjust=1, hjust=0, size=6) +
  annotate("segment", x = 3.25, y = 0.5, xend = 3.7, yend = 1.15, 
         linewidth=1, linejoin = "mitre", linetype=1, color="#004495") +
  annotate("text", x=3.5, y=1.5, label="CL[p]", parse=TRUE,
           vjust=1, hjust=0, size=6) +
  annotate("segment", x = 1, y = 0, xend = 2, yend = 0, 
         linewidth=1, linejoin = "mitre",
         arrow = arrow(type = "closed", length = unit(0.1, "npc"))) +
  coord_fixed(ratio = 1) +
  scale_x_continuous(limits=c(-1.2, 4.6), expand=c(0, 0)) +
  scale_y_continuous(limits=c(-1.8, 1.5), expand=c(0, 0)) +
  theme_void()
print(clp_vss)
```

```{r onePlot, fig.path="figs/", dev=c("png", "pdf"), fig.width=12, fig.height=3}
plot_grid(clp_vss, ivPlot, poPlot, fPlot, 
          align="v", nrow=1, labels=c("B", "C", "D", "E"))
```

```{r onePlotAlt, eval=FALSE}
ggdraw() +
  draw_plot(clp_vss, x=0, y=0, width=1/4, height=1) +
